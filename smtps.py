...
import sys, socket, string, json, datetime
...
class SMTPServerInterfaceDebug(SMTPServerInterface):
    ...
    def helo(self, args):
        print 'Received "helo"', args.replace('\n','')
        SMTPServerInterface.helo(self,args)
    
    def mailFrom(self, args):
        print 'Received "MAIL FROM:"', args.replace('\n','')
        SMTPServerInterface.mailFrom(self,args)
        
    def rcptTo(self, args):
        SMTPServerInterface.rcptTo(self,args)
        print 'Received "RCPT TO"', args.replace('\n','')

    def data(self, args):
        SMTPServerInterface.data(self,args)
        print 'Received "DATA", skipped'

    def quit(self, args):
        SMTPServerInterface.quit(self,args)
        print 'Received "QUIT"', args.replace('\n','')

    def reset(self, args):
        SMTPServerInterface.reset(self,args)
        print 'Received "RSET"', args.replace('\n','')
